    ...
    async execute(interaction) {
        ...
        if (runkey === 1) {
            ...
            const desc = (!responsekey) ?
                ...
                :
                {
                    'vi': `### Người Dùng: <@${targetid}>\n▸ **Tổng Số Lần Kiểm Tra:** \`${UserObj.total.normal + UserObj.total.special}\`\n• **Bình Thường:** \`${UserObj.total.normal}\`\n• **Đặc Biệt:** \`${UserObj.total.special}\`\n\n▸ **Max/Min Trong 100 Lệnh:**\n• **Check Đơn:** (Max: \`${(UserObj.values.run.nonavg.length > 0) ? Number(UserObj.values.max).toFixed(4) : '--'}%\` -- Min: \`${(UserObj.values.run.nonavg.length > 0) ? Number(UserObj.values.min).toFixed(4) : '--'}%\`)\n• **Trung Bình:** (Max: \`${(UserObj.values.run.avg.length > 0) ? Number(UserObj.values.maxavg).toFixed(4) : '--'}%\` -- Min: \`${(UserObj.values.run.avg.length > 0) ? Number(UserObj.values.minavg).toFixed(4) : '--'}%\`)`,
                    'en-US': `### User: <@${targetid}>\n▸ **Total Checking Usage:** \`${UserObj.total.normal + UserObj.total.special}\`\n• **Normal Cases:** \`${UserObj.total.normal}\`\n• **Special Cases:** \`${UserObj.total.special}\`\n\n▸ **Max/Min of The 100 Recent Usages:**\n• **Non-Average:** (Max: \`${(UserObj.values.run.nonavg.length > 0) ? Number(UserObj.values.max).toFixed(4) : '--'}%\` -- Min: \`${(UserObj.values.run.nonavg.length > 0) ? Number(UserObj.values.min).toFixed(4) : '--'}%\`)\n• **Average:** (Max: \`${(UserObj.values.run.avg.length > 0) ? Number(UserObj.values.maxavg).toFixed(4) : '--'}%\` -- Min: \`${(UserObj.values.run.avg.length > 0) ? Number(UserObj.values.minavg).toFixed(4) : '--'}%\`)`
                }
