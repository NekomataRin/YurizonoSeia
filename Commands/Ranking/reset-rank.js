    ...
    async execute(interaction) {
        ...
        const ChoicesButton = new ActionRowBuilder()
            .addComponents(
                new ButtonBuilder()
                    .setCustomId('yes')
                    .setLabel('| Yes')
                    .setEmoji('1384989806593048727')
                    .setStyle(ButtonStyle.Secondary),
                new ButtonBuilder()
                    .setCustomId('no')
                    .setLabel('| No')
                    .setEmoji('1138231827140706304')
                    .setStyle(ButtonStyle.Secondary)
            )
        ...
        if (!usingkey) {
            ...
            const ResetChoice = new EmbedBuilder()
                .setColor('White')
                .setAuthor({ name: `${interaction.user.username}`, iconURL: `${iuser.displayAvatarURL({ dynamic: true, size: 512 })}` })
                .setTitle('<:seiaconcerned:1244128341540208793> • Reset Rank - Notice')
            ...
            const ResetConfirm = new EmbedBuilder()
                .setColor('Red')
                .setAuthor({ name: `${interaction.user.username}`, iconURL: `${iuser.displayAvatarURL({ dynamic: true, size: 512 })}` })
                .setTitle('<:seiaconcerned:1244128341540208793> • Reset Rank - Done!')
            ...
            const ResetDoing = new EmbedBuilder()
                .setColor('Yellow')
                .setAuthor({ name: `${interaction.user.username}`, iconURL: `${iuser.displayAvatarURL({ dynamic: true, size: 512 })}` })
                .setTitle('<:seiaconcerned:1244128341540208793> • Reset Rank - Proceeding')
            ...
            const ResetReject = new EmbedBuilder()
                .setColor('Green')
                .setAuthor({ name: `${interaction.user.username}`, iconURL: `${iuser.displayAvatarURL({ dynamic: true, size: 512 })}` })
                .setTitle('<:seiaconcerned:1244128341540208793> • Reset Rank - Aborted')
            ...
            collector.on('collect', async a => {
                if (a.customId === 'yes') {
                    ...
                    await Level.deleteMany({ GuildID: interaction.guild.id, exp: 0, level: 0, total: 0 })
